...
export default function Amenities() {
  ...
  return (
    <section className="py-24 bg-resort-black text-white px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-16 md:mb-24 text-center md:text-left">
          ...
          <h2 className="font-serif text-4xl md:text-6xl font-light">
            {amenities.titlePrefix} <span className="italic text-resort-gold">{amenities.titleItalic}</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 auto-rows-[300px] gap-4 md:gap-6">
          {amenities.items.map((item, index) => (
            ...
            >
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-resort-black/90 via-resort-black/20 to-transparent opacity-80 transition-opacity duration-500 group-hover:opacity-90" />

              <div className="absolute bottom-0 left-0 p-8">
                <h3 className="font-serif text-2xl md:text-3xl text-white mb-2">{item.title}</h3>
                <p className="font-sans text-sm text-white/70 max-w-sm opacity-0 transform translate-y-4 transition-all duration-500 group-hover:opacity-100 group-hover:translate-y-0">
                  {item.description}
                </p>
              </div>
            </motion.div>
